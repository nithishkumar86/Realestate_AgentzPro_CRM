...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('agentzpro-meta-sql-' + [guid]::NewGuid().ToString('N'))
...
'@ | Set-Content -LiteralPath $bootstrap -Encoding UTF8
    ...
    $sqlFiles = @(
        ...
        (Join-Path $repositoryRoot 'scripts/sql/meta-connections-regression.sql')
    )
